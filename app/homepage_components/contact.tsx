    ...
    <form className='px-2 pb-8 md:pb-0 md:px-0' onSubmit={(e) => formHandler(e)}>
        ...
        <div className="bg-neutral-800 bg-opacity-[40%] shadow-inner relative rounded-[4px] rounded-tl-[0px] p-3 md:p-4 pb-2 md:pb-3">
            <div className="">
                ...
                <input onChange={(e) => updateFormData(e, 'name')} value={FormData.name} required type='text' placeholder='John Smith' className={`${Roboto} focus:shadow-inner pl-3 text-2xl md:text-3xl text-zinc-800 bg-zinc-100 w-full md:w-[30rem] focus:outline-none h-14 rounded-[4px]`} />
                <p className={`${Roboto1} mb-1 text-white drop-shadow-sm text-lg md:text-xl pl-2 mt-4`}>Email</p>
                <input onChange={(e) => updateFormData(e, 'email')} value={FormData.email} required type='email' placeholder='john.smith@acme.com' className={`${Roboto} focus:shadow-inner pl-3 text-2xl md:text-3xl text-zinc-800 bg-zinc-100 w-full md:w-[40rem] focus:outline-none h-14 rounded-[4px]`} />
        ...
        <div className="bg-neutral-800 bg-opacity-[40%] shadow-inner relative rounded-[4px] rounded-tl-[0px] p-3 md:p-4">
            <div className="flex flex-wrap gap-4 md:gap-6">
                <div onClick={() => setPurposeState(1)} className={`${PurposeState === 1 ? 'bg-gradient-to-br from-blue-500 to-cyan-500 shadow-md' : 'bg-sky-300 shadow-inner bg-opacity-10 hover:bg-opacity-[15%]'} transition-[color] px-0 grow md:w-auto px-14 h-16 md:h-20 rounded-[70px] border-zinc-800 hover:cursor-pointer text-zinc-100 flex justify-center items-center text-2xl md:text-3xl ${Roboto}`}>Feedback</div>
